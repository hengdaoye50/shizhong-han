...
朝代分布（约）：东汉 135+ · 西汉 57 · 汉魏之际 15 · 魏 14 · 蜀汉 11 · 吴 8 · 新 5 · 其余更始/晋等。
...
**证据三态**：`有明文` / `推断` / `阙疑`。  
**卡片 status**（库内）：`草稿` / `待审` / `已校`；统计与发表前请按 status 过滤并说明规则。
...
# http://127.0.0.1:8765/            关系图谱 + 名录
# http://127.0.0.1:8765/timeline.html 时间轴
```

部署到个人站：将 `shizhongdata/` 整目录放入 Hugo 等静态站点的 `static/` 下（例如 `static/shizhongdata/`）。
